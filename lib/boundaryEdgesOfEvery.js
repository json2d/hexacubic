...
module.exports = centers => options => {
  ...
  edges.forEach(edge => {
    ...
    for (let _edge of edges) {
      ...
      if (isEqualWith(edgeReversed, _edge, edgeComparator)) { // NOTE: shared edges will always have coordinates in reversed order
...
function edgeComparator(edgeA, edgeB) {
  return edgeA.every((corner,i) => corner.every((n,j) => F.equals(n, edgeB[i][j])));
}